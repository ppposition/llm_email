...
class EmailReceiver:
    ...
    def connect(self) -> bool:
        """连接到IMAP服务器"""
        return self.qq_email_client.connect_imap()
    
    def disconnect(self):
        """断开与IMAP服务器的连接"""
        self.qq_email_client.disconnect_imap()
    
    def _ensure_connection(self) -> bool:
        """确保连接有效"""
        return self.qq_email_client._ensure_imap_connection()
    
    def get_email_folders(self) -> List[str]:
        """获取所有邮件文件夹"""
        return self.qq_email_client.get_folders()
    ...
    def get_new_emails(self, folder_name: str = 'INBOX', since: Optional[datetime] = None) -> List[Email]:
        """获取新邮件"""
        emails = self.qq_email_client.get_new_emails(folder_name, since)
